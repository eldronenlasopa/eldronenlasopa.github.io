...
export class MensajesService {
    ...
    msgExportarVacio(msgExportar: any){
    ...
    msgLoadTimer(text: string, time?: number, callBack?: any): boolean {
        ...
        Swal.fire({
            ...
            didOpen: () => {
                ...
                const timer = popup?.querySelector("b"); // Usa el operador de encadenamiento opcional
                if (!timer) return; // Verifica si `timer` es nulo o indefinido

                // Verifica si `time` es indefinido y proporciona un valor predeterminado
                const safeTime = time ?? 1000; // Usa 1000ms como valor predeterminado si `time` es undefined

                timerInterval = setInterval(() => {
                    timeElapsed += 100;
                    const percentage = Math.round((timeElapsed / safeTime) * 100);
                    timer.textContent = `${percentage}%`;
                    if (timeElapsed >= safeTime) {
